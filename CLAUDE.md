# beautyflow-weekly-flow-check

BeautyFlow Weekly Flow Check — an internal tool that turns a salon owner's weekly answers into one prioritized bottleneck summary and next action in under five minutes.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "238493898+hamzinas98-commits@users.noreply.github.com"
  git config user.name "hamzinas98-commits"
  ```
